...
- `/skill-pdf` → [`.claude/skills/skill-pdf/SKILL.md`](.claude/skills/skill-pdf/SKILL.md) — **author estimate** (no separate verdict doc — the SKILL body holds the chain spec, expected-token table, and failure-mode notes). The chain's measurement layer is what produces every other skill's measured receipt.
