...
| `ragctl.py`                                      | The ops CLI + REPL. Stack: `status` / `watch` / `doctor` / `up` / `down` / `test` / `model` / `english` / `features` / `feature` / `usage` / `logs` / `prune` / `watchdog`. Shoutbox: `queue` / `approve` / `reject` / `reply` / `publish`.                                      |
