...
| `ragctl.py`                                      | The ops CLI + REPL. Stack: `status` / `watch` / `doctor` / `up` / `down` / `verify` / `test` / `model` / `english` / `features` / `feature` / `usage` / `logs` / `prune` / `watchdog`. `up` runs `verify` unless `--skip-verify`. Shoutbox: `queue` / `approve` / `reject` / `reply` / `publish`.                                      |
