...
3. **Task + attack gates** (pre-retrieval, deterministic, no GPU): `guardrails.is_injection_attempt` (checked first: override/reveal/persona-swap aimed AT the assistant) → `INJECTION_REPLY`; then `is_generative_request` (write a poem/story/song/**recipe**/… — a recipe is on that list because "give me a recipe for X" measures 0.33 against the 0.41 gate and the topic is irrelevant: the same ask about pizza measures 0.39), `is_personal_trivia`, and `is_translation_request` (translate X → a named language) → decline with `GENERATIVE_REPLY` (or `WEAK_RETRIEVAL_REPLY` for trivia — a missing-from-corpus fact, not an out-of-scope task). The injection gate exists because the relevance gate **cannot** catch these: the corpus is about prompts and grounding, so an attack retrieves real documents and passes as relevant. It keys on grammar (second-person imperative) not topic; the false-positive suite in `tests/test_injection_gate.py` is the half that matters.
