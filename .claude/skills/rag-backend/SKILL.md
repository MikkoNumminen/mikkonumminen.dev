...
3. **Task gates** (pre-retrieval, deterministic, no GPU): `guardrails.is_generative_request` (write a poem/story/song/…), `is_personal_trivia`, and `is_translation_request` (translate X → a named language) → decline with `GENERATIVE_REPLY` (or `WEAK_RETRIEVAL_REPLY` for trivia — a missing-from-corpus fact, not an out-of-scope task).
...
6. **Prose-anchored weak-retrieval gate** (`guardrails.is_weak_retrieval`): gates on the best **prose**-chunk cosine distance (code chunks can lower off-topic distances, so gating on prose is the honest signal); `db.closest_prose` is fetched explicitly when the top-k is all code. Threshold `WEAK_RETRIEVAL_DISTANCE=0.45`. A deterministic CV-intent override (`query_projects.wants_cv_intent`) skips the refusal when the CV route fired and CV chunks are in context. Off-corpus → fixed `WEAK_RETRIEVAL_REPLY`, no LLM call.
...
| `guardrails.py` | `is_weak_retrieval` (prose-anchored), `is_generative_request`, `is_translation_request`, language routing (`looks_finnish`, `requests_finnish_answer`), `smalltalk_route`, `is_expansion_request`. |
...
`TOP_K=6` · `WEAK_RETRIEVAL_DISTANCE=0.45` · `LLM_NUM_PREDICT=1024` · `INPUT_MAX_CHARS=800` · `LLM_MAX_CONCURRENCY=2` · `LLM_ACQUIRE_TIMEOUT_SECONDS=0.5` · `HYBRID_ENABLED=true` · `RRF_K=60` · `RETRIEVAL_DENSE_WEIGHT=1.0` · `RETRIEVAL_LEXICAL_WEIGHT=1.0` · `PROJECT_FILTER_STRICT=true` · `RAG_LOG_FILE` (empty=off) · `MAX_BODY_BYTES=16384` · `RATE_LIMIT_REQUESTS=30` · `RATE_LIMIT_WINDOW_SECONDS=60` · `FORCE_ENGLISH=true` · `CORS_ALLOW_ORIGINS` · chunk-size knobs · `CONTEXT_WINDOW=4096` (served context window; the context-bar SSE frame reports real usage against it; must be positive) · `RETRIEVAL_EXCLUDE_DOC_TYPES=adr` (comma-separated `doc_type`s hidden from retrieval; empty string disables the filter) · `RETRIEVAL_DIVERSITY_MAX_PER_PROJECT=3` (per-project chunk cap on generic, no-project-named queries; must be positive) · `RESEARCH_COVERAGE_TOP_N=3` (forces this many newest research posts into context on a recency intent; 0 disables; must be `<= TOP_K`) · `RAG_ALLOW_FINNISH=false` (experiment: answer a Finnish-looking query in Finnish instead of forcing English) · `RAG_TRANSLATE_RETRIEVAL=false` (retrieve with an LLM-translated English query when `RAG_ALLOW_FINNISH` routed Finnish; best-effort) · `PROGRESSIVE_DISCLOSURE_ENABLED=true` (the "tell me more?" offer + topic-expansion path; off restores single-shot answers) · `MEMORY_MAX_TURNS=6` / `MEMORY_MAX_SESSIONS=1000` / `MEMORY_TTL_SECONDS=1800` (session-memory bounds; all three must be positive). **The compose only passes `LLM_MODEL`; everything else uses these defaults.**
