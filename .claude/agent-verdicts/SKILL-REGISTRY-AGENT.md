...
### Why receipts can come from four places (docs/SKILLS.md, agent-verdicts/\*.md, README.md, SKILL.md body)
...
- **`with_receipts` vs annual contributors.** A semantic gap: `totals.with_receipts` counts skills with **any** receipt object (non-null), but a receipt can have non-null `path`/`source` and **null** `tokens_per_use` / `uses_per_year` / `annual_total` — those skills count toward `with_receipts` but contribute nothing to `annual_tokens_saved`. In the 2026-05-19 run after the README-source fix: **25 with_receipts** but only **19 actual contributors** (13 Spacepotatis catalog skills + 6 of AudiobookMaker's 10 with non-null annual_total; mikkonumminen.dev's 2 and 4 of AudiobookMaker's 10 don't contribute because cadence is qualitative or saves/inv is load-bearing-only). The gap got wider in absolute terms after PR #114 (was 15/13 = 2-skill gap; now 25/19 = 6-skill gap), which makes the open question more important, not less. Options: (a) tighten the schema language to make the gap explicit (done in PR #111's SKILL.md edit); (b) add a new `with_annual_total` metric to `totals` so consumers can pick either count; (c) require receipts to have non-null savings figures to qualify. Option (a) is in place; (b) and (c) deferred.
...
- [x] Missed-source caught and addressed (README.md catalog pattern, surfaced by user-feedback after first PDF render — PR #114 adds it as source (c); AudiobookMaker goes from 0 contribution to 1,786,000)
- [x] HTML-entity bug mitigated via main-thread post-process (after second occurrence proved prompt convention insufficient)
- [ ] Frontmatter schema decision made (deferred — receipt coverage is now 25/25 catalog skills via the four sources, but contributor count is 19/25; closing the 6-skill gap structurally still wants schema adoption)
