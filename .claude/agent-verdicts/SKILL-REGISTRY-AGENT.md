...
- **Sub-agent HTML-entity bug.** The AudiobookMaker sub-agent in the first run returned JSON with `&lt;` / `&gt;` inside description strings (e.g. `docs/audits/ai-smell-&lt;YYYY-MM-DD&gt;.md` instead of `<YYYY-MM-DD>`). Invalid JSON-string content; fixed during main-thread aggregation. PR #111 added an explicit "no HTML entities — bare `<` and `>` are valid in JSON strings" line to the agent prompt's conventions list. Future runs should not hit this; tracked so a regression would be recognized.
