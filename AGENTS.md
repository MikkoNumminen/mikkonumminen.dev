...
- **Output cap** — `LLM_NUM_PREDICT` (default 1024) hard `num_predict`, so no single answer can dump a document regardless of prompt content. Raised from 512 after the request log showed 169 of 2547 answers ending at exactly the cap; sized to leave headroom inside `OLLAMA_CONTEXT_LENGTH`, see `config.py`.
