...
Rounds 1-5 of my skills optimization study left six cells I did not trust: two script-backed auditor skills — `skills-quality` and `skills-freshness` — crossed with `opus`, `sonnet` and `haiku`. Each was a single draw per arm, and the same skill swung from -70% to +54% between rounds. Round 6 re-measured all six at depth: five draws per arm on both opus cells, three per arm on the four sonnet and haiku cells, arm A (cold) against arm B (with-skill), model held constant. All six came out positive — 54% to 85% saved on the cell medians. The volume-weighted aggregate is 75% saved: 748,768 tokens summed across the cold-arm medians against 188,474 with the skill, a net 560,294 saved over 6 cells. Two cells explicitly overturn their earlier verdicts, and in both the reversal is attributed to the cold arm having under-worked at N=1 — not to the skill getting better.
