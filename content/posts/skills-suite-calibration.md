...
That band is not special to the accident. Every A/B pair in this study was measured non-simultaneously, so ~8pp of cross-session drift plus N=1 task-work variance rides on every cell. So the tables colour only cells clearing ±20%; within that is direction-at-best, not a magnitude. That is why the mikko- aggregate row (+13/+15/+27) renders neutral in the source despite being positive.
