...
All three voiceovers mirror the same gate: each script reads `window.matchMedia('(prefers-reduced-motion: reduce)').matches` at load time and branches entirely, so no audio is ever loaded or played for reduced-motion users from any voice layer. Music continues because it is ambient rather than attention-demanding; the voiceover gate is deliberately separate from the music toggle.

Blog narration shipped without that gate at first, on the argument that a one-shot reading is a different thing from a clip that replays every fifty idle seconds, and that suppressing it would remove a reading aid from the group most likely to want one. The argument was reasonable and the rule was still wrong to have. One gate stated in a sentence (reduced motion silences narration, music continues) is a rule anyone can hold; an exception carved for one of three otherwise-parallel layers is a rule nobody remembers correctly, and it makes the accessibility behaviour depend on trusting a distinction the visitor never agreed to. The blog layer now takes the same gate as the other two.
...
The third one is the point at which the rule of three would normally force an extraction, and it was deliberately not done. The shared part is roughly thirty lines of play, pause and resume, while each caller keeps its own idle timer, its own volume rule and, in the blog case, its own persistence. A helper plus three call sites each overriding half of it would trade duplication a reader can see for indirection they cannot.
...
A Safari edge case in the voiceover: if the audio source file 404s (a locale without a recorded narration), Safari's `<audio>` element is in `HAVE_NOTHING` state and throws `InvalidStateError` on any `currentTime` write. Every play helper wraps `currentTime`, `volume`, and `play()` inside a `try/catch` so a missing locale file degrades silently.

Blog narration adds one thing the other two do not have: it remembers where you were. A sixteen-minute reading with no scrub control makes losing your place a real cost, so the playhead is written to `sessionStorage` on a `timeupdate` handler and restored on arrival. The key includes the locale as well as the slug, because the same post in two languages is two recordings of different lengths and one shared key would drop a Finnish offset into the middle of the English reading. Positions under five seconds are ignored (resuming into the middle of the first sentence is worse than starting over) and a finished reading clears its mark, so the next visit does not resume at the very end. Two details came out of reviewing it rather than writing it. The write is throttled to one every five seconds, because `timeupdate` fires roughly four times a second and `sessionStorage` is a synchronous main-thread write: unthrottled, one long post stored a number some 3,800 times; the paths that actually matter, toggling off and leaving the post, still write immediately. And a finished reading now refuses to restart when the sound toggle is cycled, which it previously did: without that guard the toggle quietly became a replay control, the same failure the absent idle timer exists to prevent, arriving by a different route. Every storage call is wrapped in `try/catch`: Safari in private browsing throws on `sessionStorage` access, and a convenience must never break a working player.
...
For tuning, opt-in score logging (`RAG_LOG_FILE`, empty disables it) writes one JSON line per request (the truncated query, the top cosine distances, the gate decision, and the response length), so threshold changes are driven by real numbers rather than guesses. And the whole contract is pinned by a black-box acceptance harness (`evals/acceptance.py`, run with `python -m evals.acceptance`): every static contract case plus every golden must-refuse query, covering injection-no-dump, prompt-reveal-blocked, off-topic poem and trivia declined, the input cap at 400 and the oversized-body 422, and grounded technical answers. The classifiers are anchored on the real refusal wording, so a regression that quietly changes behaviour cannot false-pass the suite.
