...
The test suite (714 tests, of which 707 run and 7 skip on a given platform, in about 10 s, measured 2026-08-14) is entirely offline. Every external effect in the watch loop has an injectable port: `clock`, `sleep`, `get_block`, `perform`, and `stop`. ccusage is mocked either by injecting `get_block` directly or by setting `CLAUDE_CONTINUE_CCUSAGE_CMD="cat tests/fixtures/active.json"`. Platform detection is overridden with `CLAUDE_CONTINUE_PLATFORM=windows|macos|wsl|linux`. All subprocess calls (`osascript`, `tmux`, `ditto`, `Popen`) are mocked in tests. Pure decision functions (`schedule.*`, `watch_explanation`, `update_decision`, `should_auto_recheck`) are tested directly without any subprocess or display.
