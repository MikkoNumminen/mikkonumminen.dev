...
export const sv: Translations = {
  ...
  timelineData: {
    ...
    kasvulabs: {
      title: 'Kasvu Labs Oy',
      body: 'Två år av full-stack-arbete, i tre faser.\n\nFörst öppen data. Jag byggde ett TypeScript-verktyg som hämtar offentlig statistik och stöper om den, och det blev kvar i bruk på företaget efter att jag slutade. Det matade en app för ett kiosknätverk: försäljning per kiosk och per produkt, vem som ansvarade för vad, och en karta över Finland med varje kiosk lagd ovanpå kommunal ålders-, inkomst- och sysselsättningsdata. Frågan den besvarade var var det borde finnas fler kiosker, och var färre.\n\nSedan kundprojektet, där jag tog full-stack-ärenden från en kanban-tavla. Mestadels frontend: React-komponenter, MUI anpassad långt bortom sina standardinställningar, och skräddarsydda lösningar där inget färdigt passade. Ofta hela vägen också, en PgTyped-fråga, REST-endpointen för den, kopplingen och vyn ovanpå, med tester.\n\nJag ägde även de månatliga datauppdateringarna. Det fanns ingen direkt databasåtkomst, så jag gick in via Kubernetes-klustret och körde psql i en pod mot produktion.\n\nDen sista fasen var ett medicinskt forskningsprojekt. Samma stack, samma form från ände till ände.',
      tags: [
        'TypeScript',
        'React',
        'Next.js',
