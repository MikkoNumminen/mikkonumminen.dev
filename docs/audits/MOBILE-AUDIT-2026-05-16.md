...
| H8 | Sound toggle (BackgroundAudio.astro) + music bed (looping, scroll-volume) | Same, toggle visible, autoplay-with-gesture path identical, scroll-volume curve identical | None viewport-based; CSS only repositions the toggle at `≤640px` | [BackgroundAudio.astro:254-257](src/components/BackgroundAudio.astro#L254-L257) |
| H9 | Landing voiceover (`HeroVoiceover.astro`), one-shot, layered on top of the music bed | **Same, voice plays on mobile too.** `<audio id="hero-voice">` mounts with no viewport gate; script listens for `bg-audio:state` and plays/pauses in lockstep with the music toggle. Once-per-session `mn_voice_landing_done` flag suppresses replay across navigations. | None viewport-based | [HeroVoiceover.astro:25-32](src/components/home/HeroVoiceover.astro#L25-L32), [HeroVoiceover.astro:51](src/components/home/HeroVoiceover.astro#L51) |
| H10 | Per-letter title pop + ripple, goat hop + dust puff, Saturn ring spin + sparkles, galaxy flare, star bloom, data-feed `$ ping mikko` injection, all clickable via `InteractionManager` | **None of these fire on mobile.** Five of the six targets live in the Three.js scene, which never boots at ≤640 px (H1). The sixth (data-feed click) is hidden under `.hero__corners { display: none }` at ≤860 px (H2/H3). Mobile receives the wiring code for `BackgroundAudio` only, every other click target is unreachable. | Consequence of H1 + H2/H3; no separate gate | [interactions.ts:155](src/lib/three/interactions.ts#L155), [homeScene.ts (registers letter/galaxy/star/goat/ring targets)](src/lib/three/homeScene.ts), [Hero.astro:425](src/components/home/Hero.astro#L425) (data-feed JS guard) |
...
- **Why this matters:** `createRenderer.ts` exposes a `maxPixelRatio` constructor option that defaults to **1.5** (the documented "nice middle ground" between sharpness and pixel-fill cost), and sets `renderer.setPixelRatio(Math.min(devicePixelRatio, maxPixelRatio))`. But `createResizeHandler.ts:21` independently calls `renderer.setPixelRatio(Math.min(devicePixelRatio, 2))` with a **hardcoded 2**, no `maxPixelRatio` plumbing. Scene modules invoke `resize.handler()` synchronously right after construction (see `homeScene.ts`), which immediately overrides the 1.5 cap with 2 before the first frame; subsequent `window.resize` events do the same. Net effect: the `maxPixelRatio` option silently has no observable effect. Today this is invisible because Three.js doesn't run on mobile; if you ship a mobile-scoped scene (per §4 "Mobile-scoped Three.js"), this will quietly burn ~78 % more pixel fill on a DPR-3 phone than the 1.5 cap promises.
