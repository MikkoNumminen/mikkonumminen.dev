...
> **⚠️ Superseded historical snapshot (2026-04-08).** This was the first audit
> pass and lived at the repo root. It is kept for history but is **out of date**:
> several findings (including both 🔴 Critical Three.js leaks: the unkilled hover
> tweens and the leaked ring geometry) have since been fixed, and line references
> point at code that has since moved. Do not action findings from this file
> without re-verifying against the current tree. The current audit of record is
> [`FULL-AUDIT-2026-05-17.md`](FULL-AUDIT-2026-05-17.md); see also the other dated
