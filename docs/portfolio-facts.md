...
once as a `14 repos` tag. Those two lagged at "nine projects" and "9 repos"
until August 2026, while the CV's entry for the same year already said
thirteen. The fourteenth (`readlog-laravel`, August 2026) meant changing all
four files plus `projectCategory.test.ts`, and no test checks that they agree.
...
**One project built three times, and deliberately listed three times**
...
- `Readlog-csharp` is a port of `ReadLog`, not a separate product. `projects.ts`
  ...
  count toward the 14. Both are deployed, which is the point worth making about
  them.
- `readlog-laravel` is a port of `Readlog-csharp` (TypeScript to C# to PHP, each
  port treating the previous one as its specification). Listed as a second moon
  of `readlog`. It is **not** hosted: it runs on the author's machine. What this
  site serves at `/readlog-laravel` is the live app, proxied from that machine
  through a Tailscale Funnel behind `api/readlog-portal.mjs` while it is on, and
  otherwise a static snapshot generated by the repo's own
  `php artisan readlog:snapshot` and committed under
  `public/readlog-laravel-snapshot/`; regenerate and copy to refresh it
  (ADR 0019). Facts as of 2026-08-23: Laravel 13, PHP 8.4, 404 Pest tests run
  against both SQLite and Postgres 16 in CI, PHPStan level 6, Google sign-in as
  the only way in with reading public and writing behind an account (its
  decisions 144 to 146), a demo-writes switch the author's deployment has on so
  a visitor can add to the showcase library (149), and one feature beyond the
  port: "ask your library", a natural-language search over the reader's entries
  through a local Ollama that degrades to the title search when Ollama is
  absent (its DECISIONS.md #94 to #103). Source:
  <https://github.com/MikkoNumminen/readlog-laravel>.
