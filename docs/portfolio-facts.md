...
- `readlog-laravel` is a port of `Readlog-csharp` (TypeScript to C# to PHP, each
  ...
  of `readlog`. It is **not** hosted: it runs on the author's machine and is
  exposed on demand through a Cloudflare tunnel. What this site serves at
  `/readlog-laravel` is a static snapshot generated by the repo's own
  `php artisan readlog:snapshot` and committed under `public/readlog-laravel/`;
  regenerate and copy to refresh it. Facts as of 2026-08-19: Laravel 13, PHP
  8.4, 308 Pest tests run against both SQLite and Postgres 16 in CI, PHPStan
  level 6, no authentication (deliberate; a demo reader switcher stands in),
  and one feature beyond the port: "ask your library", a natural-language
  search over the reader's entries through a local Ollama that degrades to
  the title search when Ollama is absent (its DECISIONS.md #94 to #103). Source:
