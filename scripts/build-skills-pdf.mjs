...
function renderCalibrationPage(measuredWithPriors) {
  ...
  <p class="calib-intro">Where the guesses come from: each skill carries an author-written estimate in its <code>SKILL.md</code> frontmatter (or in the repo's <code>README.md</code> table for older skills). I imagined what a single use should cost in tokens and wrote that number down when first scaffolding the skill, before any real run existed. No math, no benchmark — just the number that felt right at the time. The provenance for each row (<code>source: skill-body | readme.md</code> and the file path) lives on the row's <code>prior_estimate</code> block in <code>public/data/skills-registry.json</code>.</p>
